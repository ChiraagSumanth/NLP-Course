...
def f2(h,t):
	...
	'''
	if len(h[2])-2 >= h[3] and h[2][h[3]+1].lower() in products and t=="ORGANIZATION":
		return 1
	else:
		return 0
...
def f4(h,t):
	...
	if len(h[2])-2 >= h[3] and h[2][h[3]+1].lower() in money_words and t=="MONEY":
		return 1
	else:
		return 0
...
def f11(h,t):
	...
	if h[2][h[3]] in months:
		return 1

	try:
		if h[2][h[3]+1] in months:
			p = re.compile("\d{1-2}")
			m = p.match(h[2][h[3]])
			if m and (t=="TIME" or t=="DATE"):
				return 1
	except:
		pass

	if h[1] == "TIME" or h[1] == "DATE":
		p = re.compile("\d{1-2}")
		m = p.match(h[2][h[3]])
		if m:
			return 1
...
def f14(h,t):
	if(h[2][h[3]].lower() in products and t=="PERSON"):
		return 1
	else:
		return 0		

def f15(h,t):
	if(h[2][h[3]].lower() in cities and t=="LOCATION"):
		return 1
	else:
		return 0

def f16(h,t):
	m = re.search(r'[0-9]{1,2},[0-9]*',h[2][h[3]])
	if(m and t=="MONEY"):
		return 1
	else:
		return 0
